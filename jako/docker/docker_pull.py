...
def docker_pull(image_name):
    ...
    if (resp.status_code != 200):
        ...
        if (resp.status_code == 200):
            print(
                '''[+] Manifests found for this tag
                (use the @digest format to pull the corresponding image):''')
    ...
    req_url = 'https://{}/v2/{}/blobs/{}'
    confresp = requests.get(req_url.format(registry,
                            repository, config),
    ...
    empty_json = '''{"created":"1970-01-01T00:00:00Z",
        ...
        "StdinOnce":false,"Env":null,"Cmd":null,"Image":"", \
    ...
    for layer in layers:
        ...
        auth_head = _get_auth_head(
            'application/vnd.docker.distribution.manifest.v2+json')
        req_url = 'https://{}/v2/{}/blobs/{}'
        bresp = requests.get(req_url.format(registry,
                             repository, ublob),
        ...
        if (bresp.status_code != 200):
            ...
            if (bresp.status_code != 200):
                print('\rERROR: Cannot download layer')
                print(bresp.content)
                exit(1)
